...
void Config::loadConfig() {
  ...
  if (!file.is_open()) {
    throw std::runtime_error(
        "Could not open config file or file does not exist");
  }
...
void Config::displayConfig() const {
  ...
  std::cout << "Min Memory Per Process: " << minMemoryPerProcess << std::endl;
  std::cout << "Max Memory Per Process: " << maxMemoryPerProcess << std::endl;
}
int Config::getNumCpu() const { return numCpu; }

std::string Config::getScheduler() const { return scheduler; }

int Config::getQuantumCycles() const { return quantumCycles; }

int Config::getBatchProcessFreq() const { return batchProcessFreq; }

int Config::getMinIns() const { return minIns; }

int Config::getMaxIns() const { return maxIns; }

int Config::getDelaysPerExec() const { return delaysPerExec; }

int Config::getMaxOverallMemory() const { return maxOverallMemory; }
