...
Scheduler::Scheduler(Config config, std::vector<Process *> *processVector) {
  ...
  this->globalExecDelay =
      (delaysPerExecution + 1) *
      100; // IMPORTANT: This is the delay for each instruction execution
...
void Scheduler::generateDummyProcesses() {
  while (schedulerTestRunning) {
    ...
    // std::cout << "Generated Process: " << newProcess->getProcessName() << "
    // with " << newProcess << std::endl;
...
void Scheduler::runFCFSScheduler(int cpuIndex) {
  while (schedulerTestRunning || !readyQueue.empty()) {
    ...
    // std::cout << "CPU " << cpuIndex << " processed " <<
    // currentProcess->getProcessName() << " for " <<
    // currentProcess->getInstructionsDone() <<
    //           " instructions.\n";
  ...
  while (schedulerTestRunning) {
    {
      ...
      // iterate through the coreVector and print the process name and
      // instructions done
      for (int i = 0; i < numCores; ++i) {
